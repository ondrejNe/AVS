...
void cycle_display(void) {
    static int current_digit = 0;
    // Assuming a function that sets the 7-segment display to the value provided
    SEG_Send(SEG_Digits[current_digit]);
    current_digit = (current_digit + 1) % SEG_DigitCount;
    // Add a delay or use a timer to control the speed of cycling
    HAL_Delay(1000);  // Delay 1 second (example using HAL library)
}
